...
class LeftSideZone extends StatelessWidget {
  ...
  void _showBannedWallpapersPreview(BuildContext context, AppState appState) {
    if (appState.activeTab != 'Multi' && appState.activeTab != 'Custom') {
      showDialog(
        context: context,
        builder: (BuildContext context) {
          return ContentDialog(
            title: const Text('Banned Wallpapers'),
            content: const Text(
              'Banned wallpapers are only available for Multi and Custom tabs.',
            ),
            actions: [
              Button(
    ...
    final bannedWallpapers =
        appState.bannedWallpapers[appState.activeTab] ?? [];
    if (bannedWallpapers.isEmpty) {
      showDialog(
        context: context,
        builder: (BuildContext context) {
          return ContentDialog(
            title: const Text('Banned Wallpapers'),
            content: const Text('No banned wallpapers for this tab.'),
            actions: [
              Button(
                onPressed: () => Navigator.of(context).pop(),
                child: const Text('OK'),
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return ContentDialog(
          title: Text('Banned Wallpapers (${appState.activeTab})'),
          content: SizedBox(
            width: 600,
            height: 400,
            child: GridView.builder(
              ...
              itemCount: bannedWallpapers.length,
              itemBuilder: (context, index) {
                final wallpaperEntry = bannedWallpapers[index];
                final uniqueId = wallpaperEntry['uniqueId']!;
                final url = wallpaperEntry['url']!;
                return GestureDetector(
                  onSecondaryTap: () {
                    // Show context menu for unban option
                    showDialog(
                      context: context,
                      builder: (BuildContext context) {
                        return ContentDialog(
                          ...
                          actions: [
                            Button(
                              onPressed: () async {
                                ...
                                await appState.unbanWallpaper(uniqueId);
                                // Close and reopen the banned wallpapers dialog to refresh
                                Navigator.of(context).pop();
                                _showBannedWallpapersPreview(context, appState);
                              },
                              child: const Text('Unban Wallpaper'),
                            ),
                            Button(
                              onPressed: () => Navigator.of(context).pop(),
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  // Banned wallpapers and favourites buttons
                  Row(
                    children: [
                      Expanded(
                        child: SizedBox(
                          ...
                          child: Button(
                            onPressed:
                                () => _showBannedWallpapersPreview(
                                  context,
                                  appState,
                                ),
                            child: const Icon(FluentIcons.blocked, size: 16),
                          ),
                        ),
                      ),
                      ...
                      Expanded(
                        child: SizedBox(
                          ...
                          child: Button(
                            onPressed: () {
                              // Favourites functionality - empty for now
                            },
                            child: const Icon(
